...
func generateHTMLMercury(url string) (html string) {
...
func generateHTMLSelf(url string) (html string) {
	docker run -v `pwd`:/data -t readable 'http://www.cnn.com/2017/07/23/politics/anthony-scaramucci-donald-trump/index.html'
	return
...
if _, err := os.Stat("/path/to/whatever"); !os.IsNotExist(err) {
  // path/to/whatever exists
	...
	if apiKey != "" {
		return generateHTMLMercury(url)
	} else {
		return generateHTMLSelf(url)
	}
}
