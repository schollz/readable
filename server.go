...
func main() {
	fmt.Printf("\nreadable version %s\n\n", version)
	flag.StringVar(&apiKey, "key", "", "Mercury API key (default: ignore)")
	...
	flag.StringVar(&dockerImage, "image", "schollz/readable", "docker image")
	flag.Parse()
...
func generateHTMLSelf(url string) (target Response) {
	...
	log.Println("docker", "run", "--rm", "-v", dir+":/data", "-t", dockerImage, url, GetMD5Hash(url)+".json")
	_, err := exec.Command("docker", "run", "--rm", "-v", dir+":/data", "-t", dockerImage, url, GetMD5Hash(url)+".json").Output()
	if err != nil {
		log.Print(err)
	}
